...
- **Events (added 2026-08-18):** a second, standalone entity added to the same Promotional Calendar tool, deliberately mirroring the Campaigns section 1:1 (own form, own Gantt calendar, own table, own assets-needed line items) but with no cross-links to Promotions or Campaigns — it's a parallel feature, not an extension of either. New D1 table `events` (`migrations/0003_events_init.sql`, same shape as `campaigns` minus the id-prefix difference: `E01`, `E02`, ...). New routes: `GET/PUT /api/marketing/events`, `DELETE /api/marketing/events/:id`, `POST/GET/DELETE /api/marketing/events/:id/assets/:filename`. Event assets live in the same `secure-cpg-data` R2 bucket under `marketing/events/{eventId}/{filename}`. Backend helpers (`rowToEvent`, `getEvents`, `upsertEvent`, `deleteEventRow`, `putEventAsset`, etc.) and frontend JS (`loadEvents`, `submitEvent`, `renderEventsTable`, `renderEventsGantt`, `openEventDetail`, ...) in `public/ebe-promo-calendar.html` are line-for-line the same pattern as their Campaign counterparts, just renamed. Verified locally end-to-end (create/view/delete via `wrangler dev`) before this note was written. **Migration not yet applied to the remote D1 database** — run the command below before/at deploy time.
- **Master Gantt + fullscreen (added 2026-08-19):** the Master Gantt Chart now folds Events in alongside Campaigns and Promotions. Combined rows are tagged in-memory with `_kind` (`'campaign' | 'event' | 'promo'`) so click routing goes to the right detail modal (`openCampaignDetail` / `openEventDetail` / `openDetail`); Campaign rows are prefixed `★`, Event rows `⚑`. Every Gantt card (per-channel, Campaign, Event, Master) got a "⛶ Full Screen" button using the native Fullscreen API (`toggleGanttFullscreen()`, CSS via `.card:fullscreen`) — confirmed the button and click-routing work locally, but the actual `requestFullscreen()` call itself couldn't be verified because the local test ran inside an embedded preview iframe that blocks the Fullscreen permission policy (`TypeError: Permissions check failed`). This is an iframe-embedding restriction, not an app bug — it should work normally when Chris opens the deployed page directly (not embedded). Worth a real click-test after deploy.
